...
public class Painter extends GameType implements GameState
{
	...
	final static int RID_SELL_PAINT = frontend:0x012Fr;
	final static int RID_LIST_PAINT = frontend:0x0123r;
	
	final static int RID_RGB_MODE = frontend:0x0129r;
	final static int RID_HSV_MODE = frontend:0x0124r;
	...
	int lastSliderMode=SLIDERMODE_HSV;
	...
	public Painter( Player p, Osd o, Text mt, Text il, int ctrl )
	{
		...
		Menu m = osd.createMenu( buttonStyle, -0.995, 0.76, 0.10, Osd.MD_HORIZONTAL );
		
		red0 = m.addItem( new ResourceRef( RID_PAINT_SPRAY ), CMD_SPRAY, "Pintura Spray" );
		m.addItem( new ResourceRef( RID_PAINT_PART ), CMD_PART, "Pintar Pecas" );
		red1 = m.addItem( new ResourceRef( RID_PAINT_DECAL ), CMD_DECALS, "Adesivo" );
		
		m = osd.createMenu( buttonStyle, -0.995, 0.92, 0.10, Osd.MD_HORIZONTAL );
		
		m.addItem( new ResourceRef( RID_PAINT_LOAD ), CMD_LOADSKIN, "Carregar Pintura" );
		m.addItem( new ResourceRef( RID_PAINT_SAVE ), CMD_SAVESKIN, "Salvar Pintura" );
		// REMOVE THIS LINE IF NOT MWM
		m.addItem( new ResourceRef( MW_Mod:0x0098r ), CMD_COPYSKIN, "Copiar Pintura Peca" );
		...
		Button upScrollArrow = osd.createButton( btnUp, 0.96, 0.76, CMD_SCROLL_UP, "Rolar Para Cima" );
		
		Style btnDn = new Style( 0.10, 0.10, 1.0, Frontend.mediumFont, Text.ALIGN_CENTER, new ResourceRef( Osd.RID_ARROWDN ) );
		Button downScrollArrow = osd.createButton( btnDn, 0.96, 0.92, CMD_SCROLL_DOWN, "Rolar Para Baixo" );
		...
		m.addItem( "Girar", CMD_BRUSHFLIP, "TAB ou NUM 5 " );
		m.addItem( "G. Esquerda", CMD_BRUSHRCCW, "Q ou NUM 4 " );
		m.addItem( "G. Direita", CMD_BRUSHRCW, "W ou NUM 6 " );
		m.addItem( "Aumentar", CMD_BRUSHINC, "A ou NUM 8 " );
		m.addItem( "Diminuir", CMD_BRUSHDEC, "S ou NUM 2 " );
		...
		copy_button = m.addItem( "Copiar", CMD_C_COPY );
		paint_button = m.addItem( "Pintar", CMD_C_PAINT );
		...
		pa.addItem( "Vermelho ", CMD_SLIDER_R, paintRed, 0, 255, 1, null ).printValue(" %1.0f");
		pa.addItem( "Verde ", CMD_SLIDER_G, paintGreen, 0, 255, 1, null ).printValue(" %1.0f");
		pa.addItem( "Azul ", CMD_SLIDER_B, paintBlue, 0, 255, 1, null ).printValue(" %1.0f");
		
		Style btnHsv = new Style( 0.10, 0.10, 1.0, Frontend.mediumFont, Text.ALIGN_CENTER, new ResourceRef( RID_HSV_MODE ) );
		Button gotoHsv = osd.createButton( btnHsv, 0.435, 0.76, CMD_HSV, "Trocar para Palheta HSV" );
		...
		Button gotoRgb = osd.createButton( btnRgb,0.435, 0.76, CMD_RGB, "Trocar para Palheta RGB" );
		...
		hexButton = t.addItem( "#" + prepareHexString( queueHexColor ), CMD_INPUTCOLOR );
		
		Style btnList = new Style( 0.10, 0.10, 1.0, Frontend.mediumFont, Text.ALIGN_CENTER, new ResourceRef( RID_LIST_PAINT ) );
		
		Button gotoSwatches = osd.createButton( btnList, 0.90, 0.76, CMD_SWATCH, "Abrir Palhetas Cores" );
		...
		Button savePaintCan = osd.createButton( btnSave, 0.90, 0.92, CMD_SAVE_PAINT, "Salvar essa Cor a Palheta" );

		Button loadPalette = osd.createButton( btnLoad, 0.96, 0.76, CMD_LOADCOLORS, "Carregar Palheta" );
		Button savePalette = osd.createButton( btnSave, 0.96, 0.92, CMD_SAVECOLORS, "Salvar Palheta" );
		...
		Style btnSell = new Style( 0.10, 0.10, 1.0, Frontend.mediumFont, Text.ALIGN_CENTER, new ResourceRef( RID_SELL_PAINT ) );
		
		Button gotoPicker = osd.createButton( btnDn, 0.90, 0.76, CMD_COLORPICKER, "Ir ao Modo RGB" );
		
		Button clearPaint = osd.createButton( btnSell, 0.90, 0.92, CMD_CLEARCOLORS, "Limpar todas Cores" );
...
//		temporaryCan.paintColor = temporaryCan.getCanbyIndex( temporaryCan.lastCanId ).color;
	...
	public int CheckProperParent(Part p)
	{
		if(p == null)
			return 0;
		if(player.car == null || player.car.chassis == null)
			return 0;
		if(p.getCar() == player.car.chassis.getCar())
			return 1;
		return 0;
	}	
	...
	public void handleEvent( GameRef obj_ref, int event, String param )
	{
		...
		if( event == EVENT_CURSOR )
		{
			...
			if (ec == GameType.EC_LCLICK && (!(part instanceof Part) || (part instanceof Part && CheckProperParent(part))))
			{
				...
				{ 
					if( cat == GIR_CAT_PART || cat == GIR_CAT_VEHICLE )
					{
						if( paintBrush )
						{
							...
							{
								infoline.changeText( ((Decal)paintBrush).stickies + " Adesivos Restantes" ); //mirrored!
							}
						}
					}
				}
	...
	public void osdCommand( int cmd )
	{
		if( cmd == CMD_LOADSKIN )
		{
			VehicleSkinFileReqDialog d = new VehicleSkinFileReqDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Carregar Pintura", "Carregar", GameLogic.skinSaveDir, "*" );
		...
		if( cmd == CMD_SAVESKIN )
		{
			VehicleSkinFileReqDialog d = new VehicleSkinFileReqDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.FRF_SAVE, "Salvar Pintura", "Salvar", GameLogic.skinSaveDir, "*" );
			...
			if( d.display() == 0 )
			{
				...
				new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Salvar Pintura", "Pintura foi Salva." ).display();
			}
		}
		else
		...
		if( cmd == CMD_CLEARCOLORS )
		{
			...
			reallyDelete = (0 == new NoYesDialog(
		    player.controller,
		    Dialog.DF_MODAL|Dialog.DF_DEFAULTBG,
		    "Limpar todas as cores de pintura",
		    "Isso ira excluir todas as suas cores de pintura, exceto a ultima na lista. Você realmente deseja limpar todas as suas cores de pintura?"
			).display());
		...
		{
			ColorPaletteFileReqDialog d = new ColorPaletteFileReqDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Caregar Palheta Cores", "Carregar", paletteSaveDir, "*" );
		...
		{
			ColorPaletteFileReqDialog d = new ColorPaletteFileReqDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.FRF_SAVE, "Salvar Palheta Cores", "Salvar", paletteSaveDir, "*" );
			...
			if( d.display() == 0 )
			{
				...
				new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Salvar Palheta Cores", "Sua Palheta Atual foi salva em '" + filename + "'." ).display();
			}
		}
		else
		...
		{
			int typedColor;
			StringRequesterDialog d = new StringRequesterDialog( player.controller, Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.SIF_NOEMPTY, "Introduza uma nova cor (Formato: R255, G255, B255)", "" );
			if( d.display() == 0 )
			{
