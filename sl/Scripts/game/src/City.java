...
{
	final static float		SPEED_LIMIT		= 22.2;	 // 18m/s = 80km/h
	...
	final static float		ODDS_PINKS  = 0.95;
...
*/				}
		...
		else
		{
			...
			m = osd.createMenu( butt0, -1.0, -0.5, 0 );
			m.addItem( "Parar de Assistir", CMD_STOP_WATCHING );
			m.addItem( "Correr!", CMD_PARTICIPATE );
...
//			int clubIndex = GameLogic.findRacer(player)/GameLogic.CLUBMEMBERS+1;
//
//			cash[0] = 1500*clubIndex;
//			cash[1] = 2500*clubIndex;
//			cash[2] = 4500*clubIndex;
//			cash[3] =    0;
//
//			nrPrizeList = new int[n];
//
...
//			}
//
//			int p = new NrOpponentDialog( player.controller, Dialog.DF_MODAL|Dialog.DF_FULLSCREEN, pict, s, player.money ).display();
//
//			if( p < n )
//			{
...
//
//				/**
//				 * Calc max cash by club
//				 */
//				if ( player.club > 0)
//				{
//					nrMaxCash = cash[player.club];
//				}
//
//
//				Dialog dlg = new Dialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.DF_WIDE, "APOSTA", "OK;Cancel" );
//				dlg.osd.createBG( Osd.RRT_DARKEN );
//
...
//
//				m = dlg.osd.createMenu( butt1, -0.6, y + 0.48, 0.6 ,sld1 ,sldk);
//
//				m.addItem( "Valor R$", 1, nrPrize, 0, nrMaxCash, 1, null ).printValue("   %1.1f");
//				m.addItem( "Chave na chave", 0);
//
//				dlg.osd.globalHandler = dlg;
//
//				int choice = dlg.display();
//
//				nrPrize = choice;
//
//				if ( choice > 0 ) {
...
//		else
//		{
//			new WarningDialog( player.controller, Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Sem Oponentes", "Desculpe, mas ninguem quer te desafiar agora. \n Volte amanha!" ).display();
//		}
//
//		return 0;	//cancel, nincs ellenfel!
//	}

	public int selectNrOpponentOld()
	{
		//ha ures a lista, generaljunk egyet!
		//a clubb[ok]bol szedjuk az ellenfeleket, de a kocsijuk nem a nappali, hanem egy erosebb jargany lesz!
		if( nrOpponents.size() == 0 )
...
//				if (odds <= ODDS_PINKS && GameLogic.speedymen[botID[i]].enabledPinkSlips )
				if (odds <= ODDS_PINKS && player.club > 0 && Math.random() < 0.3 )
				{
					nrPrizeList[i] = cash[3];
					GameLogic.cheats += 1; // Debug
				}
				else
				if (odds <= ODDS_CASH_2)
...
//				System.log(" and the bet is:     "+nrPrizeList[i]);
				...
				if( nrPrizeList[i] == 0 )
					s[i] = rank + tmp + b.name + " - Chave na Chave";
				else
...
//				s[i] = s[i] + " - rumored best time: "+String.timeToString( b.bestNightQM, String.TCF_NOMINUTES );
	...
	public void startNightRace( int playerRace )
	{
		...
		if( nrShowRaceStart )
		{
			...
			nrDialog = new RaceDialog(player.controller, Dialog.DF_MODAL, new ResourceRef(RID_NRSTARTBG + pict), nrHead2, nrHead1, nrName2, nrName1, 1, 1, nrPrize, 0, pl2, pl1 );
...
//				changeCamTarget2(nrBot2.car);
			...
			Frontend.loadingScreen.display( nrDialog, 0.0 );
...
//				nrBot1.car.races_won =    Math.random()*nrBot1.fileid*nrBot1.aiLevel;
		...
		if( nrShowRaceFinish )
		{
			...
			Frontend.loadingScreen.display( nrDialog, 3.0 );
		}
...
//					GameLogic.changeActiveSection( parentState );
				...
				} else
				{
					...
					if( rank==1 )
					{
						...
						raceDialog = new RaceDialog(player.controller, Dialog.DF_MODAL|Dialog.DF_FREEZE, new ResourceRef(RID_DAY_WIN), player.character, raceBot.character, player.name, raceBot.name, 0.0, 0.0, prize, 0, player, raceBot );
					} else
					{
						speechDAYYOULOOSE.play();
						raceDialog = new RaceDialog(player.controller, Dialog.DF_MODAL|Dialog.DF_FREEZE, new ResourceRef(RID_DAY_LOOSE), raceBot.character, player.character, raceBot.name, player.name, 0.0, 0.0, prize, 0, raceBot, player );
					}
...
public class RacerTalkDialog extends Dialog
{
	...
	public void show()
	{
		...
		m.addItem( " 2. \"Deixa, Nao era nada nao.\"", CMD_EXIT );
...
public class FineDialog extends CsDialog
{
	public FineDialog( Controller ctrl, int myflags, String name, int[] fine )
	{
		...
		if( fine[1] )
		{
			osd.createText( "Excesso Velocidade:",		Frontend.smallFont, Text.ALIGN_LEFT,	row1, top, line ).changeColor( color );
			osd.createText( "$" + fine[1],		Frontend.smallFont, Text.ALIGN_LEFT,	row2, top, line++ ).changeColor( color );
			row1+=rc; row2+=rc;
		}
		if( fine[2] )
		{
			osd.createText( "Acidentes:",		Frontend.smallFont, Text.ALIGN_LEFT,	row1, top, line ).changeColor( color );
			osd.createText( "$" + fine[2],		Frontend.smallFont, Text.ALIGN_LEFT,	row2, top, line++ ).changeColor( color );
			row1+=rc; row2+=rc;
		}
		if( fine[3] )
		{
			osd.createText( "Perseguisoes:",		Frontend.smallFont, Text.ALIGN_LEFT,	row1, top, line ).changeColor( color );
			osd.createText( "$" + fine[3],		Frontend.smallFont, Text.ALIGN_LEFT,	row2, top, line++ ).changeColor( color );
			row1+=rc; row2+=rc;
		}
		if( fine[4] )
		{
			osd.createText( "Alteracao de Caracteristica:",	Frontend.smallFont, Text.ALIGN_LEFT,	row1, top, line ).changeColor( color );
...
//	public NrOpponentDialogOld( Controller ctrl, int myflags, ResourceRef[] pict, String[] text, int[] prizeSums, int playerMoney )
//	{
...
//	}
	...
	public NrOpponentDialog( Controller ctrl, int myflags, ResourceRef[] pict, String[] text, int playerMoney )
	{
		super( ctrl, myflags|DF_FULLSCREEN|DF_DARKEN|DF_FREEZE, null, null );
...
public class NightRaceDialog extends Dialog
{
	...
	{
		super(ctrl, DF_DEFAULTBG|DF_DARKEN|DF_MODAL|DF_FREEZE, "The following racers are willing to race with you:", "CANCEL");
