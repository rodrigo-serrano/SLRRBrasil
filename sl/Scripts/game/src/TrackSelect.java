...
// OSD stuff:
	...
	// Create Styles:
		...
		osd.createHeader( "Selecionar Pista" );
		...
		FoundTrackText = osd.createText( "Mapas: " + MapsAmount, Frontend.largeFont, Text.ALIGN_CENTER, 0.0, 0.915 ); FoundTrackText.changeColor(0xFFC0C0C0);// "Unselected" gray
		...
		OkButton = m.addItem( "Correr!", CMD_OK );
		OkButton.enable();
		...
		if (MapsAmount > 10)// If more than 10 track found
		{
			...
			UpButton = m.addItem( new ResourceRef( frontend:0x004Cr ), CMD_UP, "Pag Ant" );
		
			m = osd.createMenu( Menu_Button, 0.0, 0.60, 0, Osd.MD_VERTICAL );
			DownButton = m.addItem( new ResourceRef( frontend:0x0084r ), CMD_DOWN, "Prox Pag" );
	...
	public void osdCommand( int cmd )
	{
		...
		if( cmd == CMD_OK )
		{
			...
			{
				new WarningDialog( GameLogic.player.controller, Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Aviso", "Voce Precisa de um Carro." ).display();
			}
		}
		else
		if( cmd == CMD_ABOUT )
		{
			new WarningDialog( GameLogic.player.controller, Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.DF_WIDE, "Selecionar De Pistas v1.1 By Miran" ).display();
		}
		else
		if( cmd == CMD_UP )
		{
