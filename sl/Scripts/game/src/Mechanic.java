...
public class Mechanic extends GameType
{
	...
	public Mechanic( Player p, Osd o, Text mt, Text il, int ctrl )
	{
		...
		osd.createButton( btnUp, 0.96, 0.76, CMD_SCROLL_UP, "Rolar Para cima" );

		Style btnDn = new Style( 0.10, 0.10, 1.0, Frontend.mediumFont, Text.ALIGN_CENTER, new ResourceRef( Osd.RID_ARROWDN ) );
		osd.createButton( btnDn, 0.96, 0.92, CMD_SCROLL_DOWN, "Rolar Para baixo" );
...
//		osd.createRectangle( -1, 1, 2.0, 0.46, 4.0/3.0, 0.5, -0.5, -1, new ResourceRef(frontend:0x00C0r) );
		...
		m.addItem( new ResourceRef( RID_REPAIR_PART ), CMD_REPAIR, "Reparar Peca" ).enableDrop();
		m.addItem( new ResourceRef( RID_REPAIR_CAR ), CMD_REPAIR_ALL, "Reparar o Carro" );
		m.addItem( new ResourceRef( RID_SELL_PART ), CMD_SELL, "Vender Peca" ).enableDrop();

		m = osd.createMenu( buttonStyle, -0.995, 0.92, 0.10, Osd.MD_HORIZONTAL );

		m.addItem( new ResourceRef( RID_FILTER_ENGINE ), CMD_ENGINE, "Mostrar/Esconder Pecas Motor" );
		m.addItem( new ResourceRef( RID_FILTER_BODY ), CMD_BODY, "Mostrar/Esconder Pecas Lataria" );
		m.addItem( new ResourceRef( RID_FILTER_RGEAR ), CMD_RUNNING_GEAR, "Mostrar/Esconder Pecas Suspensao" );
	...
	public void handleEvent( GameRef obj_ref, int event, String param )
	{
		...
		if( event == EVENT_CURSOR )
		{
			...
			if (ec == GameType.EC_LCLICK)
			{
				...
				if( part instanceof Part && CheckProperParent(part))
				{
					if( mode == 0 )
					{	//szereles mode
	...
	public void osdCommand( int cmd )
	{
		...
		if( cmd == CMD_REPAIR )
		{
					...
					if( part )
					{
						if( part instanceof Chassis )
						{
							...
							if( reason != 0 )
							{
								...
								new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Chassis", beginsWith + " Montado ao Chassis. \n Todas Pecas precisam estar desmontadas para Reparar o Chassis." ).display();
								part=null;
							}
						}
						else
								...
								if( fcost > 0 )
								{
									...
									if( cost <= player.money )
									{
										...
										if( cost )
											...
											d = new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Informacao", part.name + " Foi Reparado de Graca." );
									...
									else
									{
										...
										new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Peca", "Voce Precisa de $" + money_need + " A mais para Reparar!" ).display();
									}
								}
								else
								{
									new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Peca", "Essa Peca ja foi Reparada!" ).display();
								}
							}
							else
							{
								new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Peca", "Essa Peca virou Sucata." ).display();
							}
						}
						else
						{
							new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Peca", "Essa Peca esta 0km!" ).display();
						}
					}
				}
			}
			...
			{
				new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Informacao", "Drope Pecas aqui para Repara-las! \n \n Voce tambem pode Dropar O Chassis Direto Aqui." ).display();
			}
		}
		else
		if( cmd == CMD_SELL )
		{
			...
			else				//click
			{
				if (inventory.items.size() >= 1)
				{
					...
					if( 0 == new YesNoDialog( player.controller, Dialog.DF_LOWPRI|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Vender Todas Pecas", "Deseja Vender todas " + parttype + " pecas por $" + totalprice + "?" ).display() )
					{
				...
				{
					new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Informacao", "Clique aqui para vender todas suas pecas, ou drope elas individualmente aqui. \n \n Para Vender seu Carro visite a Revenda de Carros Usados." ).display();
				}
			}
		}
		else
						...
						if( fcost > 0 )
						{
							...
							if( cost <= player.money )
							{
								...
								if( cost )
									...
									d = new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Informacao", "Seu Carro foi Reparado de Graca." );
							...
							{
								new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Peca", "Voce precisa de $" + (cost-player.money) + " A mais!" ).display();
							}
						}
						else
						{
							new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Carro", "Esse Carro ja esta Reparado!" ).display();
						}
					}
					else
					{
						new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Carro", "Esse Carro virou Sucata!" ).display();
					}
				}
				else
				{
					new WarningDialog( player.controller, Dialog.DF_FREEZE|Dialog.DF_MODAL|Dialog.DF_DEFAULTBG, "Reparar Carro", "Esse Carro esta 0km!" ).display();
				}
			}
		}
	}
...
public class ScrapDialog extends YesNoDialog
{
	...
	{ 
		super( ctrl, myflags, "Vender Peca", "Deseja Vender essa " + item.getName() + " Por $" + price + " ?"); 
	}
}
...
public class RepairDialog extends NoYesDialog
{
	...
	{ 
		super( ctrl, myflags, "Reparar Peca", "Vai te custa $" + cost + " Para reparar essa " + p.name + ". Deseja Reparar?" ); 
	}
}
...
public class RepairAllDialog extends NoYesDialog
{
	...
	{ 
		super( ctrl, myflags, "Reparar Carro", "Vai te custa $" + cost + " Para reparar esse Carro. Deseja Reparar?" ); 
	}
}