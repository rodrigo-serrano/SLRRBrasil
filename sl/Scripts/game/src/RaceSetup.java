...
//			Frontend.loadingScreen.hide();
		...
		{
			if( i==0 )
				prizeValues[i] = "Apenas Prestigio";
			else
...
//		if( GameLogic.gameMode == GameLogic.GM_QUICKRACE )
	...
	public void updatePrize()
	{
		...
		if( prizeText )
			prizeText.changeText( "Premio: " + prizeValues[prizeMultiplier] );
	}

	public void createOSDObjects()
	{
		osd.createHeader( "Configuracao Corrida" );
		...
		if( GameLogic.player.prestige >= opponent.prestige )
			osd.createText( "Voce Escolhe a rota.", Frontend.mediumFont, Text.ALIGN_LEFT,	-0.95, 0.92 );
		else
			osd.createText( opponent.name+" Escolhe a rota.", Frontend.mediumFont, Text.ALIGN_LEFT,	-0.95, 0.92 );
		...
		m.addItem( new ResourceRef( Osd.RID_CANCEL ), CMD_ABANDON, "Abandonar Corrida" );
		m.addItem( new ResourceRef( Osd.RID_OK ), CMD_RACE, "Correr!" );
		if( GameLogic.gameMode != GameLogic.GM_QUICKRACE )
		{
			m.addSeparator();
			Gadget g = m.addItem( new ResourceRef( frontend:0x00B4r ), CMD_PRIZE, "Trocar a Aposta" );
			prizeText = osd.createText( null, Frontend.mediumFont, Text.ALIGN_RIGHT,	0.95, -0.98);
		}
		m.addSeparator();
		m.addItem( new ResourceRef( frontend:0x006Ar ), CMD_ZOOM_OUT, "Zoom +" );
		m.addItem( new ResourceRef( frontend:0x0061r ), CMD_ZOOM_IN, "Zoom -" );
