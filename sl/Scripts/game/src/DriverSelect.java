...
// OSD stuff:
	...
	// Create Styles:
		...
		DownButton = m.addItem( new ResourceRef( frontend:0x00AFr ), CMD_DOWN, "Prox Pag" );
		...
		TakeButton = m.addItem( "Selecionar", CMD_OK );

		osd.createText( "Trocar Motorista", Frontend.largeFont, Text.ALIGN_CENTER, 0.01, -0.980 ).changeColor(0xFFC0C0C0);// "Unselected" gray
		ModelInfo = osd.createText( "-Nenhuma Motorista Selecionado-", Frontend.largeFont, Text.ALIGN_CENTER, 0.01, 0.920 ); ModelInfo.changeColor(0xFFC0C0C0);// "Unselected" gray

		m = osd.createMenu( Menu_Button, -0.35,  0.95, Osd.MD_VERTICAL ); 
		m.addItem( new ResourceRef( frontend:0x0085r ), CMD_PREV, "Motorisa Anterior" );

		m = osd.createMenu( Menu_Button, 0.35,  0.95, Osd.MD_VERTICAL ); 
		m.addItem( new ResourceRef( frontend:0x0088r ), CMD_NEXT, "Proximo Motorista" );
	...
	public void osdCommand( int cmd )
	{
		...
		if( cmd == CMD_ABOUT )
		{
			new WarningDialog( GameLogic.player.controller, Dialog.DF_MODAL|Dialog.DF_DEFAULTBG|Dialog.DF_WIDE, "Selecionar Motorista" ).display();
		}
		else
		if( cmd == CMD_NODRIVER )
		{
			...
			ModelInfo.changeText("Nenhum Motorista Selecionado");
		}
		else
		if( cmd == CMD_DEFAULT )
		{
