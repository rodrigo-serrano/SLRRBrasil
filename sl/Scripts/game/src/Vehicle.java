...
//  		System.log( "Vehicle created " + chassis.vehicleName + " " + (System.currentTime()-t) );
	...
	{	
		...
		else if (GameLogic.player.car)
		{
			return "NONE";
		}

		return "Ai Seu Noia, Aprende a fazer um VT!";
	}
	...
	public String isDriveable()
	{
		...
		if (error)
			return "Voce nao pode dirigir esse carro por causa de " + error;
		return null;
	}

	public String getHp()
	{
		if( !chassis || isTestable())
		{
			return "N/A";
		}

		return Float.toString(chassis.maxHP/1.256, "%1.0f HP");
	}
	...
	public String isTestable()
	{
		...
		if( chassis )
		{
			...
			if( !engine )
				return "Nao tem Motor.";
			else
			if( engine.dynodata.maxTorque <= 0.0)
				return "Motor nao esta Funcionando.";

			if (chassis.starter_torque == 0.0)
			{
				return "Seu Motor de Partida esta Ruim.";
			}

			if( !battery )
			{
				return "Falta a bateria.";
			}

			if( !swheel )
				return "Nao tem volante.";

			if( !drvseat )
				return "Nao tem Banco do Motorista.";
		}
	...
	public String isDriveable2()
	{
		...
		if( chassis )
		{
			...
			if ( wheels == 0 && chassis.wheels > 0 )
				return "Carro nao tem Rodas.";
			else
			if ( wheels < chassis.wheels-3 )
				return "Falta alguns Rodas.";
			else
			if ( wheels == chassis.wheels-3 )
				return "Faltam 3 Rodas no Carro.";
			else
			if ( wheels == chassis.wheels-2 )
				return "Faltam 2 Rodas no Carro.";
			else
			if ( wheels == chassis.wheels-1 )
				return "Falta uma Roda no Carro.";
		}
