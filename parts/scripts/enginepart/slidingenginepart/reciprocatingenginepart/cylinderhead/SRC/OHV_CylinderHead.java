...
public class OHV_CylinderHead extends CylinderHead
{
	...
	public Part getCamshaft()
	{
		...
		int parentSlotId = getSlotID(-1);
		Part res = partOnSlot(parentSlotId);

		if(!res)
			return null;

		if (res instanceof Block_Vee_OHV)
			return ((Block_Vee_OHV)res).getCamshaft();

		if (res instanceof Block_Inline_OHV)
			return ((Block_Inline_OHV)res).getCamshaft();
