...
//		System.log(">>> Valve timing for "+name+" end <<<");
	...
	public String advanceCapabsToText()
	{
		if (advance_positive_peak-advance_negative_peak>0.0)
			return "Pode ser Ajustado "+Float.toString(advance_negative_peak,"%1.1f")+Float.toString(advance_positive_peak," de %1.1f Graus")+Float.toString(advance_minimum_step," em %1.1f Graus");
		return "Nao pode ser Ajustado";
	}
	...
	{
		old_advance = advance;
		m.addItem( "Avanco Polia",			1, advance, advance_negative_peak, advance_positive_peak, (advance_positive_peak-advance_negative_peak)/advance_minimum_step+1, null ).printValue("   %1.1f deg.");

		m.addItem( "Resetar",		0);	//this should always be with cmd=0
	}
