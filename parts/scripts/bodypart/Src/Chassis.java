...
//	float	table_stepsize = 500;	//torquetable stepsize
	...
	int	brake_balance_can_be_set = 1;

	Vector	exhaustSlotIDList = null;
	
	Vector	PneumaticSA, AnimateParts;
...
//	float	sfx_horn_pitch	=    1.0;
	...
	public void Refresher_Start()
	{
		for( int i=0; i<=AnimateParts.size()-1; i++ )
		{
			AnimateParts.elementData[i].Refresher_Start();
		}
	...
	public void Refresher_Stop()
	{
		...
		for( int i=0; i<=AnimateParts.size()-1; i++ )
		{
			AnimateParts.elementData[i].Refresher_Stop();
		}
	}
	...
	/*public void run()
	{
		for(;;)
		{
			if(the_car)
			{
				for( int i=0; i<=AnimateParts.size()-1; i++ )
				{
					//AnimateParts.elementData[i].Stop();
					AnimateParts.elementData[i].Refresh();
				}
			}
			else{
				AnimateThread.sleep(20);
			}
		}
	}*/
	...
	public Part getBlock()
	{
		Part p = partOnSlot(ENGINE_SLOT);
		if(p && p instanceof Block) return p;
...
//			dyno.LogVars();
			...
			//if(engine.inertia >= 9.0 && engine.inertia <= 16.0)
			//{
		...
		} else
		{
			...
			SFX_ignition	=sound:0x0000000Ar;
			rpm_trans_fwd	= 1400.0;
			rpm_trans_rev	= 1400.0;
			sfx_starter_rpm	=  200.0;
...
//		setDifflock(diff_lock);
	...
	public void buildTuningMenu( Menu m )
	{
		...
		if (brake_balance_can_be_set)
			m.addItem( "F-R brake balance",		1, -brake_balance, -1.0, 0.0, 101, null ).changeVLabelText( Float.toString(brake_balance*100.0, "%1.1f %%"));
	}
	...
	public float getCurrentTurboPressure()
	{
		
		float CurrentRPM = getIndexedData( IndexedData_RPM );
		float acel = getIndexedData( IndexedData_ThrottleStrength );
		float v2 = 0.0;
		float v3 = 0.0;
		float v4 = 0.0;
	...
	public float getCurrentTurboPressure2()
	{
