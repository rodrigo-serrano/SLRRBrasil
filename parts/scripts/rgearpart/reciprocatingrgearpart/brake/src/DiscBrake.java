...
public class DiscBrake extends Brake
{
	...
	public void calcStuffs()
	{
		...
		if (friction_disc == RT_GROOVED)
			rotor_mat_text="Frisado";
		else
		if (friction_disc == RT_VENTEDGROOVED)
			rotor_mat_text="Ventilado";
		else
		if (friction_disc == RT_CARBON)
		{
			rotor_mat_text="Carbono";
			police_check_fine_value = 100;
		}

		String caliper_mat_text = "";
		if (force == CL_FORCE_1)
			caliper_mat_text="De Rua";
		else
		if (force == CL_FORCE_BRUTAL)
			caliper_mat_text="Esportivo";
		else
		if (force == CL_RACEFORCE)
		{
			caliper_mat_text="De Pista";
			police_check_fine_value = 100;
		}

		String pad_mat_text = "";
		if (friction_pad == BP_RED_STUFF)
			pad_mat_text="Composto de Rua";
		else
		if (friction_pad == BP_GREEN_STUFF)
			pad_mat_text="Composto Verde";
		else
		if (friction_pad == BP_BLACK_STUFF)
			pad_mat_text="Composto Black";

		name = name_prefix + " " + dmmI+"mm "+number_of_calipersI+" piston disc brake";
		description = "Um disco de freio utilizando "+dmmI+"mm "+rotor_mat_text+" Rotor, "+number_of_calipersI+" "+caliper_mat_text+" Pistoes e "+pad_mat_text+" Pastilhas. Torque de frenagem: "+torque+" Nm.";
	}
}
