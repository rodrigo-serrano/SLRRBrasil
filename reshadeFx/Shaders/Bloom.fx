...
uniform int iLensdirtMixmode <
	...
	ui_items = "Linear add\0Screen add\0Screen/Lighten/Opacity\0Lighten\0";
> = 1;
uniform bool bLensdirtMirrorEffect<
> = false;
...
void LightingCombine(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 color : SV_Target)
{
	...
	if (bLensdirtEnable)
	{
		float lensdirtmult = 0;
		if(bLensdirtMirrorEffect)
		{
			lensdirtmult = dot(tex2D(SamplerBloom5, (-1*texcoord)+1).rgb, 0.333);
		}
		else
		{
			lensdirtmult = dot(tex2D(SamplerBloom5, texcoord).rgb, 0.333);
		}
