...
> = 1000.0;
uniform float FarCurveMultiplierIfInGarage <
	ui_type = "drag";
	ui_min = 0.01; ui_max = 5.0;
	ui_tooltip = "FarCurveMultiplierIfInGarage";
> = 1.0;
uniform float InfiniteFocusMultiplierIfInGarage <
	ui_type = "drag";
	ui_min = 0.01; ui_max = 5.0;
	ui_tooltip = "InfiniteFocusMultiplierIfInGarage";
> = 1.0;
...
float GetCoC(float2 coords)
{
	...
	float real_DOF_INFINITEFOCUS = DOF_INFINITEFOCUS;
	if(SpeedOfCar>10000000)
		real_DOF_INFINITEFOCUS*=InfiniteFocusMultiplierIfInGarage;
	scenefocus = smoothstep(0.0, real_DOF_INFINITEFOCUS, scenefocus);
	scenedepth = smoothstep(0.0, real_DOF_INFINITEFOCUS, scenedepth);
	float real_DOF_FARBLURCURVE = DOF_FARBLURCURVE;
	if(SpeedOfCar>10000000)
		real_DOF_FARBLURCURVE*=FarCurveMultiplierIfInGarage;
	float farBlurDepth = scenefocus * pow(4.0, real_DOF_FARBLURCURVE);
...
void PS_RingDOF1(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 hdr2R : SV_Target0)
{
	...
	if(SpeedOfCar<10000000)
		discRadius *= min((1.0/((SpeedOfCar+1.0)/SpeedDivider)),1.0);
