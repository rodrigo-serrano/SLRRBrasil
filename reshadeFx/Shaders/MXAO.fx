...
uniform float AddIfInGarage <
	ui_type = "drag";
	ui_min = 0.01; ui_max = 5.0;
	ui_tooltip = "AddIfInGarage";
> = 1.0;
...
if(bMXAOSmartSamplingEnable) numSamples = lerp(iMXAOSampleCount,12,scenedepth / AO_FADE____END); //AO FADEOUT //12 is minimum acceptable sampling count and max(12,...) makes falloff ineffective for small sample counts.
	...
	float realfMXAONormalBias = fMXAONormalBias;
	if(SpeedOfCar>10000000)
		realfMXAONormalBias+=AddIfInGarage;
	[loop]
		for (float i=1.0; i <= numSamples; i++) 
	{
		...
		if(currentOffset.x < 1.0 && currentOffset.y < 1.0 && currentOffset.x > 0.0 && currentOffset.y > 0.0)
		{
			...
			float fAO = saturate(occlDistance * fNegInvR2 + 1.0)  * saturate(SurfaceAngle - realfMXAONormalBias); 	
	...
	AOandGI *= 20.0 / ((1.0-realfMXAONormalBias)*numSamples*fMXAOSampleRadius); 
